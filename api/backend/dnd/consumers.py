...
class InitiativeConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        self.group_name = "initiative_tracker"
    ...
    async def receive(self, text_data):
        # DM will send updates, handled in views.py
        pass

    async def send_initiative_update(self, event):
        data = event["data"]
        await self.send(text_data=json.dumps({
